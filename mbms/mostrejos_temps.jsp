...
try {
    ...
    if (request.getParameter("especie").equals("-")) {
	    ...
	    "  m.ubicacion_id = su.id  order by year) p";
    	 ...
    	 myQuery = "select json_agg(p) from (select su.nombre_ubicacion," +
				 " r.ind_transecto+r.ind_paseo+r.m_transecto +r.m_paseo +r.f_transecto +r.f_paseo + r.f_transecto_2 +r.f_transecto_3 +r.ind_transecto_2 +r.ind_transecto_3 +r.m_transecto_2 +r.m_transecto_3+r.f_transecto_4+r.f_transecto_5+r.ind_transecto_4+r.ind_transecto_5+r.m_transecto_4+r.m_transecto_5 as suma," +
    		    " case when EXTRACT(DAY FROM fecha) < 16 then '1a quinzena'  else '2a quinzena' end as quinzena,"+
    		    " case when EXTRACT(MONTH FROM fecha) = 3 then 'Març' "+
    		     " when EXTRACT(MONTH FROM fecha) = 4 then 'Abril' "+
    		    " when EXTRACT(MONTH FROM fecha) = 5 then 'Maig' "+
